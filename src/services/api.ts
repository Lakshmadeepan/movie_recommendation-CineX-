...
export const FALLBACK_MOVIES: Movie[] = [
  ...
  },
  {
    ...
    runtime: "2h 44m",
    overview: "A mild-mannered café owner becomes a local hero through an act of violence, which sets off a chain of events that connects him to a dangerous drug cartel from his past.",
    ...
    cast: [
      { name: "Vijay", role: "Parthiban / Leo Das", photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&h=200&fit=crop&auto=format" },
      { name: "Trisha Krishnan", role: "Sathya", photo: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=200&h=200&fit=crop&auto=format" },
      { name: "Sanjay Dutt", role: "Antony Das", photo: "https://images.unsplash.com/photo-1519345182560-3f2917c472ef?w=200&h=200&fit=crop&auto=format" },
    ],
    streaming: [{ name: "Netflix", logo: "N", color: "#E50914" }],
  },
  {
    ...
    runtime: "2h 48m",
    overview: "A retired jailer goes on a manhunt to find his son's killers and uncovers a massive idol smuggling network spanning across the country.",
    ...
    cast: [
      { name: "Rajinikanth", role: "Muthuvel Pandian", photo: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&h=200&fit=crop&auto=format" },
      { name: "Mohanlal", role: "Mathew", photo: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&h=200&fit=crop&auto=format" },
      { name: "Shiva Rajkumar", role: "Narasimha", photo: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=200&h=200&fit=crop&auto=format" },
    ],
    streaming: [{ name: "Amazon Prime", logo: "▶", color: "#00A8E1" }],
  },
];
...
export async function getMovies(params?: { genre?: string; language?: string; sortBy?: string }): Promise<Movie[]> {
  const query = new URLSearchParams();
  if (params?.genre) query.set("genre", params.genre);
  if (params?.language) query.set("language", params.language);
  if (params?.sortBy) query.set("sort_by", params.sortBy);

  const url = `${API_BASE}/movies${query.toString() ? `?${query.toString()}` : ""}`;
  return fetchJson<Movie[]>(url, FALLBACK_MOVIES);
}

export async function getTrendingMovies(): Promise<Movie[]> {
  return fetchJson<Movie[]>(`${API_BASE}/movies/trending`, FALLBACK_MOVIES.slice(0, 8));
}

export async function getTopRatedMovies(): Promise<Movie[]> {
  const fallback = [...FALLBACK_MOVIES].sort((a, b) => b.rating - a.rating);
  return fetchJson<Movie[]>(`${API_BASE}/movies/top-rated`, fallback);
}

export async function getTamilMovies(): Promise<Movie[]> {
  const fallback = FALLBACK_MOVIES.filter((m) => m.language === "Tamil");
  return fetchJson<Movie[]>(`${API_BASE}/movies/tamil`, fallback);
}

export async function getMovieById(id: number): Promise<Movie | null> {
  const fallback = FALLBACK_MOVIES.find((m) => m.id === id) || null;
...
export async function getRecommendations(movieId: number, limit = 6): Promise<Movie[]> {
  ...
  const fallback = targetMovie
    ? FALLBACK_MOVIES.filter(
        (m) => m.id !== movieId && (m.language === targetMovie.language || m.genres.some((g) => targetMovie.genres.includes(g)))
      ).slice(0, limit)
    : [];

  return fetchJson<Movie[]>(`${API_BASE}/movies/${movieId}/recommendations?limit=${limit}`, fallback);
}
...
export async function searchMovies(query: string): Promise<Movie[]> {
  ...
  return fetchJson<Movie[]>(`${API_BASE}/search?q=${encodeURIComponent(query)}`, fallback);
}
